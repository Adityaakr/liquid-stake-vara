...
try {
  ...
  await page.waitForFunction(() => document.body.innerText.includes('live'), null, { timeout: 30000 }).catch(() => {});
  ...
  expect(/Deposit APY\s*\n?\s*(7\.9|8\.4)%/.test(text), 'APY comes from the program config');
  ...
  expect(stake.includes('coming soon') && stake.includes('Open the vaults'), 'stake page shows the coming soon panel on chain');
