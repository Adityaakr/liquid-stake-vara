...
# Usage: scripts/gen-clients.sh   (after `cargo build --release` in programs/demo-token and programs/vault)
set -euo pipefail
cd "$(dirname "$0")/.."
...
for f in src/chain/idl/demo_token.ts src/chain/idl/vault.ts; do
